 ...
 */
export function TargetStatus({ targetHp, totalHp, helmet, armor, currentHelmetDurability, currentArmorDurability }) {
    ...
    return (
        <div className="target-status-panel">
            ...
            <div className="hp-bar-container">
                ...
                <div className="hp-text">{targetHp.toFixed(1)} / {totalHp}</div>
            </div>
            ...
            <div className="status-display">
                ...
                <div className="status-item">
                    ...
                    <span className="value">{helmet ? `${currentHelmetDurability?.toFixed(1) || 'N/A'}` : '无'}</span>
                </div>
                
                {/* 护甲耐久度显示 */}
                <div className="status-item">
                    <span className="label">当前护甲耐久:</span>
                    <span className="value">{armor ? `${currentArmorDurability?.toFixed(1) || 'N/A'}` : '无'}</span>
                </div>
            </div>
        </div>
    );
