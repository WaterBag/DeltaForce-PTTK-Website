...
#### 🔧 配件系统
- 新增通用“槽位解锁 / 槽位依赖”机制：`effects.unlockSlots` + `requiresSlots`，用于实现“选择某配件解锁新增槽位”的通用玩法。
- 多页面接入统一选择逻辑：支持同槽位互斥、未解锁禁用、移除解锁源时的级联移除。

#### 🔫 数据更新
- **MCX-LT**：新增变体武器与配件
  - 变体配件：`MCX-LT-焰魂枪管`（切换伤害模板/变体数据，且解锁“外罩”槽位）
  - 配件：`SUR隔热外罩`（射速 +0.064，且仅在解锁“外罩”槽位后可装）
  - 配件：`猎手枪管`（纯百分比：射程 +30%，初速到 760；并对射程上限哨兵值 999 保持不缩放）
- **P90**：新增 `P90长弓精钢枪管`（使用变体射程模板 + 百分比射速/初速修正）。
- **弹药**：新增 4 种 SUB 弹药（7.62*39mm：BP-SUB/AP-SUB；.300BLK：V-SUB/BCP-SUB）。

### 🐛 Bug 修复 (Fixes)
- 修复 **AS-Val-刺客高级枪管** 在枪械选择中误显示的问题（变体武器不应作为可选基础枪展示）。
- 修复变体配件链路不一致导致的变体切换失效问题（`damageChange/specialRange` 字段与变体名匹配规则）。
- 修复数据图鉴点击进入时可能出现的 `toFixed` 崩溃：对变体武器实施 Minimal Variant 回退并对展示数值做兜底。

### 🧩 行为调整 (Changed)
- 数据图鉴：配件面板不再展示“变体配件”（变体武器已作为独立条目展示）。
- 数据图鉴：变体武器条目默认继承其对应变体配件的 `unlockSlots`（例如 `MCX-LT-焰魂枪管` 可直接装外罩类配件）。

### 🛠️ 维护性 (Chore)
- 为前后端核心链路的关键变量补充注释（配件/变体/TTK 计算/后端查询与版本判定），提升可读性与后续维护效率。

## [1.4.5] - 2026-02-04

### 🐛 Bug 修复 (Fixes)
- 修复“变体射程档位不足”时的 PTTK 距离映射：超出档位的 BTK 点固定到最后有效射程，避免中段插入导致一段 PTTK 异常。
- 补齐 P90 变体条目的基础属性字段，确保变体数据完整一致。

### 📝 注释 (Docs)
- 为变体射程映射逻辑补充注释，说明单调递增与回退策略。

## [1.4.6] - 2026-02-04

### ✨ 新增功能 (Features)
- 模拟器与配件弹窗弹药下拉：按穿透等级从高到低排序；默认优先选择 5 级弹药（无则 4 级）。
- 弹药下拉支持展示 `description` 小字注释（右侧居中、溢出省略）。
- 模拟器头盔/护甲默认选择 5 级耐久最高装备。

### 🔫 数据更新 (Data)
- 为多种弹药补充赛季限定注释（S7/S8）。
- `.300BLK` 的 `BCP FMJ` 更名为 `BCP-FMJ`。
- 绑定 **MCX-LT** 与 **MK4** 的图片资源（含对应变体）。

### 🧹 维护性 (Chore)
- 修复前端 lint 警告（未使用变量、Hook 依赖与缩进问题）。
